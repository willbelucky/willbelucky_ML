...
def run_training(flags, class_number, label_profit):
    ...
    with tf.Graph().as_default():
        ...
        units_placeholder, labels_placeholder = placeholder_inputs(flags.batch_size, data_sets.column_number)
        ...
        logger.info("")
        logger.info(" ".join(['step', 'training_precision', 'validation_precision', 'test_precision']))
        ...
        for step in range(flags.max_steps + 1):
            ...
            if step % (flags.max_steps / 100) == 0:
                ...
                test_num_examples, test_true_count, test_precision = do_eval(sess,
                                                                             ...
                                                                             data_sets.test,
                                                                             flags)
                logger.info("{:d}\t{:f}\t{:f}\t{:f}".format(step, training_precision, validation_precision,
                                                            test_precision))
